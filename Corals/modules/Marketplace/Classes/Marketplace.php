    ...
    public function setTransactions($invoice, $order)
    {
        ...
        if ($vendor_require_subscription) {
            ...
            if ($marketplaceSubscriptionProduct && $marketplaceCommisionFeatureId) {
                ...
                $subscriptionCommision = $storeSubscription->plan->features()->where('feature_id',
                    $marketplaceCommisionFeatureId)->first();
                if ($subscriptionCommision) {
...
//our_change new function    
     ...
     */
    //our_change add only
    public function getCategoriesList($selected = [], $excluded = [], $only = [])
    {
        $categories = Category::query();
            ...
            case 'select':
                ...
                switch ($field->getProperty('display_type')) {
                    case 'color':
                        ...
                        $input = CoralsForm::radio('options[' . $field->id . ']', $field->label,
...
//check if module installed
     public function isModuleInstalled($modele)
     {
      if( Module::where('code', $modele)->where('installed',1)->first())
       return true;
       return false;
     }

//our_change new function
public function getAttributeByKey($fieldKey, $sku = null, $attributes = [])
{

    $field = Attribute::where('properties', 'like', '%"key":"'.$fieldKey.'"%')->first();
   if($field){
    return $this->renderAttribute($field, $sku , $attributes);
   }

}
//our_change
public function getAttributeID($fieldCode){
    $field = Attribute::where('code','order authentication')->first();
    if($field){
        return $field->id;
       }
