...
{
    use PresentableTrait, ReviewRateableTrait, Wishlistable, Sluggable, LogsActivity, InteractsWithMedia ;
    ...
    public function scopeActive($query)
    {
       ...
       //our_change added
     public function scopeInfluencer($query)
     {
         return $query->where('marketplace_stores.is_influencer', true);
     }
    ...
    {
        $classification = "";
        $no_completed_orders = $this->completedOrders()->count();
        if( $no_completed_orders >= 5 ){
            $classification = 'Expert Seller';
        } else   if( $no_completed_orders >= 2 ){
            $classification = 'Trusted Seller';
        }
