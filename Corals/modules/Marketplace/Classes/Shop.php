...
class Shop
{
    ...
    protected function productsPublicBaseQuery($store = null)
    {
      ...
      //our_change add approved scope
        $query = Product::active()->approved()->join('marketplace_sku', 'marketplace_sku.product_id', '=', 'marketplace_products.id')
        ...
        switch ($sortOption) {
            ...
            case 'average_rating':
                $products = $products->leftJoin('utility_ratings', 'reviewrateable_id', '=', 'marketplace_products.id')
                    ...
                    ->addSelect(\DB::raw('ROUND(AVG(rating), 2) as averageReviewRateable'))->orderBy('averageReviewRateable',
                        'desc');
                break;
            case 'a_z_order':
...
//        });
    ...
    public function getProductsBasedOnViewedCategories($limit = 10)
    {
     ...
     //our_change
     protected function soldOutQueryBuilderFilter($products)
    {
         $products = $products->has('activeSKU', '>=', 1)
         ->where('marketplace_products.status', 'active')
        ->where('marketplace_products.type', 'simple')
         ->has('soldOut', '>=' , 1);
